...
- (void) extractFormToken: (NSDictionary*) serverResponse onExtractFormCompletion:(onGetPaymentContextCompletion) onGetPaymentContextCompletion {
    ...
    if (formToken != nil) {
        ...
        NSString * message = [[NSString alloc] initWithFormat:@"Error Code: %@ \n Error Message: %@ \n Error Detail Code: %@ \n Error Detail Message: %@", errorCode, errorMessage, detailErrorCode, detailErrorMsg];
        NSError *error = [[NSError alloc] initWithDomain:@"com.lyra.server.communication" code:1 userInfo: @{NSLocalizedFailureReasonErrorKey: message}];
        onGetPaymentContextCompletion(false, nil, error);
    } else {
        NSError *error = [[NSError alloc] initWithDomain:@"com.lyra.server.communication" code:2 userInfo: @{NSLocalizedFailureReasonErrorKey: @"Invalid formToken"}];
        onGetPaymentContextCompletion(false, nil, error);
    }
}
