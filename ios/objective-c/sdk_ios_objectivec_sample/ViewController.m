...
- (IBAction)executeSdkPayment:(id)sender {
  ...
  [_serverComunication getProcessPaymentContext:^(BOOL getContextSuccess, NSString *formToken, NSError *error) {
    if (!getContextSuccess || formToken == nil) {
      ...
      NSString *errorMessage = error != nil ? [[error userInfo] objectForKey:NSLocalizedFailureReasonErrorKey]
                                            : @"Error getting payment context";
...
- (void)verifyPayment:(LyraResponse *)lyraResponse {
  [_serverComunication verifyPayment:lyraResponse
                      withCompletion:^(BOOL paymentVerified, BOOL isErrorConnection) {
                        if (paymentVerified)
