...
class ServerCommunication {
  ...
  func getPaymentContext(
    onGetContextCompletion: @escaping (_ getContextSuccess: Bool, _ formToken: String?, _ error: NSError?) -> Void
  ) {
  ...
  func verifyPayment(
    _ lyraResponse: LyraResponse,
    onVerifyPaymentCompletion: @escaping (_ paymentVerified: Bool, _ isErrorConnection: Bool) -> Void
  ) {
    // Build request
  ...
  private static func extractFormToken(
    _ serverResponse: [String: Any], completion: @escaping (Bool, String, NSError?) -> Void
  ) {
    if let formToken = serverResponse["formToken"] as? String {
      ...
      completion(
        false, "",
        NSError(domain: "com.lyra.server.communication", code: 1, userInfo: [NSLocalizedFailureReasonErrorKey: message])
      )
    } else {
      completion(
        false, "",
        NSError(
          domain: "com.lyra.server.communication", code: 2,
